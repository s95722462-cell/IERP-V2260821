...
const TableEngine = (() => {
  ...
  function buildDom(state) {
    ...
    if (opts.dateFilter) {
      toolbarHtml += `
        <input type="month" class="te-month" data-role="month" title="선택한 달의 1일~말일이 자동으로 채워집니다">
    ...
    const monthInput = wrap.querySelector('[data-role="month"]');
    ...
    if (monthInput) {
      monthInput.addEventListener('change', () => {
        ...
        dateFrom.value = toStr(first);
        dateTo.value = toStr(last);
        // 월을 고르면 날짜 두 칸만 채워두고, 실제 조회는 아래 조회 버튼으로 실행
      });
    }
